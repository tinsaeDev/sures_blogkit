...
const urlFor = (source) => urlBuilder({ projectId: 'replace with ur value', dataset: 'production' }).image(source);

const serializer = {
  types: {
    mainImage: (props) => (
      <figure>
        <img src={urlFor(props.node.asset).width(600).url()} alt={props.node.alt} />
...
export default function SingleBlog() {
  ...
  const Fetchdata = () => {
    const lastCommentTimestamp = comments.length > 0 ? comments[comments.length - 1].timestamp : null;
    db.collection('comments')
      .where('story', '==', slug)
                      ...
                      {isLoading ? (
                        ...
                        <section>
                          ...
                          <div className="blog-item-meta  bg-gray py-1 px-2">
                            ...
                            <span className="text-muted text-capitalize mr-3">Published On</span>
                            <span className="text-black text-capitalize mr-3">
                              <i className="ti-time mr-1" /> {format(new Date(singlePost.publishedAt), 'yyyy-MM-dd')}
                            </span>
                          </div>
                          <div className="block__content">
                            <h1>{singlePost.title}</h1>
                            <PortableText blocks={singlePost.body} serializers={serializer} />
                          </div>
                        </section>
                      )}
                      <SocialShare />
